...
/**
 * @title GitcoinPassportDecoderShape
...
contract GitcoinPassportDecoderShape is
  ...
  PausableUpgradeable
{
  ...
  function _getPassportFromScoreV2(
    ...
    for (uint256 i = 0; i < stamps.length; ) {
      ...
      // START This is different for Shape
      credentials[i].hash = keccak256(abi.encodePacked(stamps[i].provider, attestation.recipient));
      // END
   ...
   * @dev This is modified on the Shape chain and will be deprecated in the future
  ...
  function getPassport(
    ...
    // START This is different for Shape
    bytes32 scoreV2AttestationUID = gitcoinResolver.communityScoreAttestations(
      7922,
      user
    );
    // END
