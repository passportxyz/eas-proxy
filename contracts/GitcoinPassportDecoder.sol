  ...
  PausableUpgradeable
{
  ...
  event PassportDecoded(Credential[] passport);
  ...
  function getAttestation(bytes32 attestationUID) public payable virtual returns (Attestation memory) {
  ...
  function getPassport(address userAddress, bytes32 schemaUID) public returns (Credential[] memory) {
    ...
    // Populate the passport memory array
    for (uint256 i = 0; i < mappedProviders.length; i++) {
      bit = 1;
      uint256 provider = uint256(providers[i]);
      for (uint256 j = 0; j < 256; ) {
        console.log('hashes[hashIndex]', mappedProviders[i]);
        ...
        if ((provider) & bit > 0) {
          ...
          credential.provider = mappedProviders[i];
          ...
          passportMemoryArray[i] = credential;
          hashIndex += 1;
        }
        bit <<= 1;
        unchecked {
    ...
    emit PassportDecoded(passportMemoryArray);
