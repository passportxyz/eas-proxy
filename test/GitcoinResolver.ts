...
describe("GitcoinResolver", function() {
  ...
  before(async function() {
    ...
    const GitcoinAttester = await ethers.getContractFactory("GitcoinAttester", owner);
    ...
    const GitcoinResolver = await ethers.getContractFactory("GitcoinResolver", owner);
    gitcoinResolver = await GitcoinResolver.deploy();
    await gitcoinResolver.connect(owner).initialize(
      mockEas.address,
      gitcoinAttesterAddress
    );
  ...
  describe("Attestations", function() {
    it("should make 1 attestation", async function() {
      const attestResult = await gitcoinResolver.connect(mockEas).attest(this.validAttestation);
    ...
    it("should make multiple attestations", async function() {
      const result = await gitcoinResolver.connect(mockEas).multiAttest([this.validAttestation, this.validAttestation, this.validAttestation], []);
      console.log(result);
    ...
    it("should revert when a non-allowed address attempts to make any attestation", async function(){  
      await expect(gitcoinResolver.connect(iamAccount).attest(this.validAttestation)).to.be.revertedWith("Only EAS contract can call this function");
    });
    ...
    describe("Revocations", function() {
      it("should make 1 revocation", async function() {
        ...
        const attestResult = await gitcoinResolver.connect(mockEas).revoke(this.validAttestatio);
      ...
      it("Should make multiple revocations", async function() {
        ...
        await gitcoinResolver.connect(mockEas).multiAttest([this.validAttestation, this.validAttestation, this.validAttestation], []);
        // Get the result of the multiRevoke
        const result = await gitcoinResolver.connect(mockEas).multiRevoke([this.validAttestation, this.validAttestation, this.validAttestation], []);
        // Destructure the events from the awaited results
      ...
      it("should allow a user to revoke their own attestations", async function() {
        ...
        const result = await gitcoinResolver.connect(recipient).revoke(this.validAttestation);
        // Destructure the events from the awaited results
        const { events } = await result.wait();
